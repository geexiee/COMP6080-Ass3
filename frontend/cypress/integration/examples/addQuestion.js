context('Add question path', () => {
  it('Goes through the add question path', () => {
    ...
    // verify that we were automatically logged in and redirected to the dashboard
    ...
    // verify that we were automatically redirected to the dashboard after successfully creating new game
    expect(cy.location('pathname').should('eq', '/dashboard'));
    ...
    // verify that we were automatically redirected back to the edit game screen after successfully adding question to game
    cy.location('pathname').should('match', /edit\/\d+/);

    // check that our newly inputted question is there
